...
\section{Background}
The Point Cloud Library includes many data types and numerous algorithms that make working with point clouds extraordinarily easy.  The first algorithm used in this research was the 3-D voxel grid filter, which downsamples point cloud data by modeling the input dataset with a three-dimensional grid having cubic cells of a user-supplied width.  Each cell that contained points in the original set is then filled with a single point placed at the centroid of the points within that part of the input (\hyperref[bib:rusu]{Rusu}). \\
The library also provides a Euclidean cluster detection routine, which is able to identify individual objects within a particular cloud.  Using a searchable representation of the input cloud, the algorithm starts with the first point and creates a processing queue that is prepopulated with the set of neighbors falling within a sphere of set radius.  Each of these points is then processed---if it has not already been---in the same manner until the entire queue has been processed; at this point, all the points that have been processed are added to a structure representing the first cluster.  The algorithm continues with the next unprocessed cloud point, and repeats until it has processed all the points in the cloud (\hyperref[bib:tuerker]{Tuerker}).

\section{Approach}
\begin{figure}
\begin{tikzpicture}[start chain=going below, every node/.style={draw,on chain,font=\scriptsize}, every on chain/.style=join, every join/.style=->, node distance=12pt]
...
{ [start chain=drive going right]
	\node[shape=ellipse,xshift=-3cm,yshift=-11cm]{Has the user enabled movement?};
	{ [start branch=gosomewhere going right]
		\node{Send the navigation commands calculated in the main thread};
		{ [every node/.style=formatting]
			\path(drive-end) edge node[above]{yes} (drive/gosomewhere-end);
		}
		\draw[->](drive/gosomewhere-end) .. controls (0,-12cm) .. (drive-1);
	}
	\draw[->](drive-end) .. controls (-2.5cm,-12cm) and (-3.5cm,-12cm) .. (drive-1);
}
{ [every node/.style=formatting]
	\node[xshift=-3cm,yshift=-11.65cm]{no};
}
\end{tikzpicture}
\caption{The final program's flow of control}
\label{fig:runtime}
\end{figure}
...
\phantomsection
\label{bib:tuerker}
Tuerker, Serkan. (2011). Euclidean Cluster Extraction. Point Cloud Library Tutorials. July 26, 2012. $\langle$\url{http://pointclouds.org/documentation/tutorials/cluster_extraction.php}$\rangle$.
\end{sloppypar}
