...
\end{tikzpicture}
\caption{An overview of the progression of code development}
\label{fig:progressionbrief}
\end{figure}

\subsection{The height range cropping algorithm}
...
\subsection{Experiments with cluster detection}
...
\subsection{The ground plane edges algorithm}
...
\subsection{Combining the height range and ground plane approaches}
...
The overall process of development is covered briefly by \autoref{fig:progressionbrief}, but a more detailed representation of the code's progression is given in \autoref{fig:progressiondetailed}.  The test cases noteworthy enough to have prompted implementation changes are collected in \autoref{tab:tests}.  Additionally, the pseudo code for the final project is discussed in \autoref{fig:runtime}.
...
\end{tikzpicture}
\caption{The complete progression of code development by week}
...
\end{tikzpicture}
\caption{The thread implementing the height range cropping algorithm}
\label{fig:runtimerange}
\end{subfigure}
...
\end{tikzpicture}
\caption{The thread implementing the ground plane edges algorithm}
\label{fig:runtimeedges}
\end{subfigure}
\begin{subfigure}{\textwidth}
\begin{tikzpicture}[formatting/.style={font=\scriptsize,execute at begin node={\begin{varwidth}{5cm}\begin{center}},execute at end node={\end{center}\end{varwidth}}}, every node/.style={draw,on chain,formatting}, every on chain/.style=join, every join/.style=->, node distance=12pt]
{ [start chain=going below]
...
\end{tikzpicture}
\caption{The thread integrating the decisions of the two separate algorithms and issuing appropriate drive commands}
